    ...
    <div className="flex flex-col h-full items-center bg-gradient-to-b from-muted/50 to-background border-r border-border/40 backdrop-blur-md">
      ...
      <ScrollArea className="flex-1 w-full px-2" hideScrollBar>
        <div className="flex flex-col gap-4 items-center py-4">
            {servers?.map((server) => (
              <div key={server.id} className="relative group w-full flex justify-center">
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    ...
                    >
                      ...
                      <span className={cn(
                        ...
                        server.status === 'connecting' ? "bg-blue-500" : "bg-muted-foreground"
                      )}>
                  ...
                  <TooltipContent side="right" className="font-medium flex flex-col gap-1 z-50 ml-3 bg-popover/95 backdrop-blur-sm shadow-xl border-border/50">
            ...
            <Tooltip delayDuration={0}>
              ...
              <TooltipContent side="right" className="ml-3">{t('mcp.servers.actions.add')}</TooltipContent>
            </Tooltip>
        </div>
      </ScrollArea>
      ...
      <div className="mt-auto flex flex-col gap-4 items-center pb-6 px-2 w-full">
         ...
         <Tooltip delayDuration={0}>
            ...
            <TooltipContent side="right" className="ml-3">{t('settings.title')}</TooltipContent>
          </Tooltip>
      </div>
